			...
			<div class="flex flex-col justify-between items-end">
				<nav class="border-2 p-4 rounded-lg flex items-center space-x-4">
					...
					<div>
						...
						<h3 class="text-xs" v-if="chatType !== 'state'">{{ friendStatus }}</h3>
					</div>
				</nav>
...
<!-- 						<input type="submit" value="Отправить файл">
 -->					</form>
			...
			<h1 class="text-red-900 text-lg" v-if="banned">Мы заблокировали ваш чат так как был найден контент характеризующий терроризм</h1>
			<div class="flex flex-col justify-start" v-if="!banned" style="min-width: 580px; overflow-x: hidden;">
				<div class="flex flex-col border-l-2 overflow-y-scroll overflow-x-hidden mb-4 space-y-2 chatCont p-8 border-t-2 rounded-lg" style="max-height: 400px;">
					
					<div v-if="loaded" v-for="(message, index) in messages" :key="index" 
					:class="[messagesSTyle,message.sender == currentUser ? ownMessagesStyle : 0]"
					style="max-width: 330px">
						<h2 class="text-md">{{ message.text }}</h2>
						<h2 @click="fileLink(message.image)" 
						class="cursor-pointer truncate text-md text-white underline font-bold" 
						v-if="message.image.endsWith('.docx') || message.image.endsWith('.pdf') 
						|| message.image.endsWith('.pptx')" >
							Скачать файл
						</h2>
						<h6 class="text-xs">{{ message.time.slice(0,5) }}</h6>
						<img @click="reqFull" v-if="message.image !== 'NO IMAGE' && !message.image.endsWith('.mp4') && !message.image.endsWith('.pptx') && !message.image.endsWith('.docx') && !message.image.endsWith('.pdf')" :src="require(`@/assets/uploads/${message.image}`)" alt="">
				...
				<div class="flex items-stretch  w-100" v-if="chatType !== 'state'">
					<input type="text" @keypress.enter="sendMessage" placeholder="Введите сообщение..." style="outline: none;" class="w-full p-4 border-2 border-r-0 rounded-full rounded-r-none"
...
<script>
	...
	export default {
		...
		data: () => ({
			...
			banWords: ['сирия','автомат','ячейки','оружие', 'ярок','аслиха',"силох"],
			banned: false,
			loaded: false

		}),
		beforeMount() {
			this.friendName = this.$route.params.friend
			this.loadChatData()			
			// this.loadChats()
		},
		methods: {
			...
			terrorFilter() {
				this.banWords.forEach(word => {
					if(this.messageInput.includes(word)) {
						this.banned = true
					}
				});
			},	
			...
			sendMessage() {
				if(this.messageInput !== '' || this.chatType == 'state' && this.appType !== '') {
					this.terrorFilter()
					console.log('Zayniddin !!!!!!!!')
					let time = new Date().toLocaleTimeString() 	 		

					let obj = null

					if(this.chatType !== 'state') {
						obj = {text: this.messageInput, time: time, sender: this.currentUser, receiver: this.friendName, image: 'NO IMAGE'}
					} else {
						obj = {text: this.appType, time: time, sender: this.currentUser, receiver: this.friendName, image: 'NO IMAGE'}
					}
					...
					if(this.chatType == 'state') {
						objResponse = {text: 'Мы успешно получили ваше заявление. В скором времени специалисты рассмотрят его и донесут до вас состояние вашей заявки. Спасибо !', time: time, sender: this.friendName, receiver: this.currentUser, image: 'NO IMAGE'}
					}
			...
			fileLink(href) {
				location.href = `http://localhost:7777/getFile/${href}`
			}
